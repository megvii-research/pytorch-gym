...
for i in range(image_num):
    ...
    tmp = cv2.resize(tmp, dsize=(image_width, image_width), interpolation=cv2.INTER_CUBIC)
...
class CanvasEnv:
    ...
    def reset(self):
        imgid = np.random.randint(image_num)
        self.target = img[imgid]
        self.stepnum = 0
        # target should be a 3-channel colored image of shape (H,W,3) in float64
        self.target_drawn = False
        self.canvas = np.zeros(shape=self.target.shape, dtype='uint8') + 255
        self.time = np.zeros(shape=self.target.shape, dtype='float64')
    ...
    def diff(self):
        ...
        # return np.sum(np.logical_xor(p, q).astype('float64')) / image_width / image_width
    ...
    def step(self, action):
        ...
        g = np.maximum(g, self.canvas.astype('float64') - 255)
        # 255 >= c - g >= 0
        # c >= g && g >= c - 255

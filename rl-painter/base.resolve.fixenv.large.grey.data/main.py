        ...
        if (done or (episode_steps >= max_episode_length and max_episode_length)): # end of episode
            ...
            if step > args.warmup:
                ...
                if episode > 0 and validate_interval > 0 and episode % validate_interval == 0:
                    ...
                    if args.env == "Paint":
                        writer.add_image(str(step) + 'canvas.png', cv2.cvtColor(env.canvas, cv2.COLOR_GRAY2RGB))
                        writer.add_image(str(step) + 'target.png', cv2.cvtColor(env.target, cv2.COLOR_GRAY2RGB))

...
def train(num_iterations, agent, env, evaluate, args, bullet):
    ...
    while step < num_iterations:
        ...
        #if args.pic:
        #    observation = cnn.get_feature_from_pic(observation)
        episode_memory.append(observation)
        observation = episode_memory.getObservation(window_length, observation)
...
if __name__ == "__main__":
    ...
    if args.pic:
        ...
        for_size.append(sp[2])  # c
        for_size.append(sp[0])  # h
        for_size.append(sp[1])  # w
    #    cnn = CNN(input_c, input_h * args.window_length, input_w, args.pic_status)
    # else:
    ...
    agent = DDPG(nb_status, nb_actions, args, for_size)
